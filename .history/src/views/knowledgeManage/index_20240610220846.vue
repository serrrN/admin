    ...
    <div class="table-content">
      <el-table :data="tableData" style="width: 100%" border>
        ...
        <el-table-column label="创建时间">
          <template #="{ row }">
            <span>{{ dayjs(row.createTime).format('YYYY-MM-DD HH:ss') }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="updateTime" label="上次修改时间">
          <template #="{ row }">
            <span>{{ dayjs(row.updateTime).format('YYYY-MM-DD HH:ss') }}</span>
          </template>
        </el-table-column>
  ...
  <el-dialog v-model="dialogVisible" title="添加知识点" width="500">
    <el-form :data="knowledgeData">
      ...
      >
        ...
        <div>
          <el-select v-model="tag" placeholder="Select" style="width: 240px">
            <el-option
          ...
          <el-button icon="Plus" style="margin-left: 15px" @click="addTag" />
        </div>
      </div>
    </el-form>
    <template #footer>
...
//添加已有的标签
const addTag = () => {
  knowledgeData.value.tags = knowledgeData.value.tags + ' ' + tag.value
  tag.value = ""
}
</script>
